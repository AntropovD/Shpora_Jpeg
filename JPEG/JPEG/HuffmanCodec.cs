...
namespace JPEG
{
    ...
    class HuffmanCodec
    {
        ...
        {
            var nodes = new HashSet<HuffmanNode>(Enumerable.Range(0, byte.MaxValue + 1).Select(num => new HuffmanNode { Frequency = frequences[num], LeafLabel = (byte)num }).Where(node => node.Frequency > 0));
            while (nodes.Count > 1)
            {
